	...
	<view class="wrap">
		...
		<view class="identity-box">
		    <view class="label">营业期限</view>
				<u-checkbox
				    v-model="companyInfo.isForever"
				    @change="checkboxChange"
				>
				    长期有效
				</u-checkbox>
				<view
				    v-if="!companyInfo.isForever"
				    class="u-text-right"
	...
	computed: {
		disabled() {
			if (this.companyInfo.businessLicense && 
					...
					(this.companyInfo.businessDeadline || this.companyInfo.isForever)
			) {
				return false
			}
	...
	methods: {
		getCompanyInfo() {
			try {
				...
				console.log('companyMess', companyMess)
				
				if (companyMess) {
					this.companyInfo.companyName = companyMess.companyName
					this.companyInfo.contactName = companyMess.contactName
					this.companyInfo.address = companyMess.address
					this.companyInfo.corporationTaxNumber = companyMess.corporationTaxNumber
					this.companyInfo.establishDate = companyMess.establishDate
					this.companyInfo.businessDeadline = companyMess.businessDeadline
					this.companyInfo.isForever = companyMess.isForever
				}
			} catch (e) {
				// error
			}
		...
		checkboxChange() {
				this.companyInfo.isForever = !this.companyInfo.isForever
				if(this.companyInfo.isForever) {
		...
		handleSubmit() {
			...
			this.$u.route(`/pages/mine/identity/company-sign?companyName=${this.companyInfo.companyName}`)
		},
	},
}
</script>
