<template>
	<view class="page-wrap" v-if="detail">
		<view>
			...
			<view class="list">
				<view class="card" v-if="otherDisputes && otherDisputes.status === 1">
				...
				<view class="card">
					...
					<view class="card-item">
						...
						<view class="card-content" @click="openReasonPopup">
							...
							<u-icon name="arrow-right" color="#cccccc" size="32" v-if="myDisputes.status !== 2"></u-icon>
						</view>
					</view>
		...
		<view class="btn" @click="save" v-if="detail.secondAuctionEntity.status === 5">提交申请</view>
		...
		<AuctionDisputesDescribePopup
			...
			:type="type"
			:disputes="disputes"
			:status="detail.secondAuctionEntity.status"
	...
	methods: {
		async getProductDetailData () {
		  uni.showLoading({
			...
			const res = await this.$getRequest(this.$url.getAuctionDetail, "GET", params)
			uni.hideLoading()
			if (res.code === 0) {
				this.detail = res.data
				if (res.data.biddingDisputesEntityList.length) {
					const myDisputes = res.data.biddingDisputesEntityList.find(item => item.fromSys === 1)
					if (myDisputes) {
						this.myDisputes = myDisputes
					}
					
					const otherDisputes = res.data.biddingDisputesEntityList.find(item => item.fromSys === 2 && item.status === 2)
					if (otherDisputes) {
		...
		async save() {
			...
			const params = {
				...
				finalPrice: this.detail.currentPrice,
			}
			...
			if (res.code == 0) {
				uni.reLaunch({
					url: `/pagesOrder/auction/detail?id=${this.id}`
				})
			}
		},
	},
