...
export default {
	...
	onLoad(options) {
		...
		// })
		// #endif
		console.log('TUI-chat | setMessageRead  | ready')
		uni.$TUIKit
			.setMessageRead({
				conversationID
			})
		...
		uni.$TUIKit.getConversationProfile(conversationID).then(res => {
			console.log('getConversationProfile', res)
	...
	methods: {
		...
		goBack() {
			...
			if (
				...
				pages[pages.length - 2].route === 'pages/home/certcar/detail'
			) {
				uni.navigateBack({
					delta: 2
				});
